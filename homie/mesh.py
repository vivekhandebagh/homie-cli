...
class MeshManager:
    ...
    def fetch_bundle_from_peer(self, peer_mesh_ip: str, auth_token: str,
        ...
        try:
            ...
            print(f"[Bundle] Connection to {peer_mesh_ip} timed out")
            print(f"[Bundle] Make sure inviter has 'homie up --mesh' running")
            return None
        except ConnectionRefusedError:
            print(f"[Bundle] Connection refused by {peer_mesh_ip}")
            print(f"[Bundle] Make sure inviter has 'homie up --mesh' running")
            return None
        except Exception as e:
            print(f"[Bundle] Failed to fetch: {type(e).__name__}: {e}")
            return None
        finally:
            sock.close()
